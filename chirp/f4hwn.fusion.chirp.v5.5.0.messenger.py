...
#                _mem.int_500tx = int(element.value)
            ...
            # messenger (kamilsss655 port)
            elif elname == "msg_receive":
                _mem.msg_receive = int(element.value)

            elif elname == "msg_ack":
                _mem.msg_ack = int(element.value)
                ...
                # bytes, so K1 and K5 derive the same 256-bit key.
                # An empty field restores the factory-erased slot (0xFF)
                key = str(element.value).rstrip()
                if key:
                    key = (key + "_" * 10)[:10] + "\x00" * 6
                else:
                    key = "\xff" * 16
                _mem.enc_key = key
...
#            top.append(dtmfc)
    ...
    # basic.append(listname_setting)
        ...
        # Messenger (kamilsss655 port)
        val = RadioSettingValueBoolean(_mem.msg_receive)
        MsgRxSetting = RadioSetting("msg_receive", "Messenger RX (MsgRX)", val)
        MsgRxSetting.set_doc('MsgRX: Listen for incoming FSK text messages')

        val = RadioSettingValueBoolean(_mem.msg_ack)
        ...
        MsgEncSetting.set_doc('MsgEnc: encrypt outgoing messages (ChaCha20) with the key derived\n' + \
                              'from the password below. The receiving radio must have the same password')

        enckey = str(_mem.enc_key).strip("\x00\xff").rstrip("_")
        enckey = _getstring(enckey.encode('ascii', errors='ignore'), 0, 10)
        val = RadioSettingValueString(0, 10, enckey)
        MsgKeySetting = RadioSetting("msg_enc_key", "Messenger encryption password (EncKey, 10 characters max)", val)
        MsgKeySetting.set_doc('EncKey: password used to derive the messenger encryption key.\n' + \
